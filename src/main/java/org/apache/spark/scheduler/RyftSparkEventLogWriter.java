 ...
 *
 * <p>This class implements {@link SparkListenerInterface} to capture and process Spark events. It
 * uses a {@link RollingEventLogFilesWriter} to manage the event log, supporting features such as
 * rolling files, retention policies, and customizable configurations. Events are serialized to JSON
 * format using Spark's {@link JsonProtocol} before being logged.
 *
 * <p>Key features include:
 *
 * <ul>
 *   <li>Integration with Spark configuration options for log directory and retention settings.
 *   <li>Automatic retries for initialization with exponential backoff.
 *   <li>Selective logging to manage logging frequency and avoid log spam.
 * </ul>
 *
 * <p>Usage:
 *
 * <p>The listener can be attached to a SparkContext to begin logging events. Configuration options
 * such as log directory, maximum file size, and retention policies are specified via Spark
 * properties prefixed with <code>spark.eventLog.ryft</code>.
 *
 * <p>Example Spark properties:
 *
 * <ul>
 *   <li><code>spark.eventLog.ryft.dir</code>: Base directory for Ryft event logs.
 *   <li><code>spark.eventLog.ryft.rolling.maxFileSize</code>: Maximum size of each log file.
 *   <li><code>spark.eventLog.ryft.rolling.overwrite</code>: Whether to overwrite existing log
 *       files.
 *   <li><code>spark.eventLog.ryft.rotation.interval</code>: Interval for log rotation.
 * </ul>
 *
 * <p>Note: If initialization fails after a predefined number of attempts, the writer will stop
 * retrying. This behavior is logged for troubleshooting.
 *
 * <p>Thread safety: This class is not thread-safe and should be used in the context of a
 * single-threaded Spark listener execution model.
 *
...
public class RyftSparkEventLogWriter implements SparkListenerInterface {
  ...
  private static final Duration DEFAULT_ROLLING_INTERVAL_MS =
      Duration.ofMinutes(5); // 5 minutes in ms
  ...
  private URI generateEventLogDirURI(String eventLogBaseDir) {
    eventLogBaseDir =
        eventLogBaseDir
            + "/"
            + StringRandomizer.generateUniqueString(EVENT_LOG_DIR_SUFFIX_LENGTH)
            + "/";
  ...
  private void writeApplicationStart() {
    LOG.info(
        "Ryft starts writing logs for application with ID: {}, and name: {}",
        applicationId,
        appName);
    try {
  ...
  // we support the following interval expressions as config values following the scala duration
  // standard:
      ...
      var maxFileSize =
          sparkConf
              ...
              .getOrElse(
                  () -> {
                    LOG.warn(
                        "Max file size is not set. Using default value: {}",
                        DEFAULT_ROLLING_FILE_MAX_SIZE);
      ...
      var minFileSize =
          sparkConf
              ...
              .getOrElse(
                  () -> {
                    LOG.warn(
                        "Min file size is not set. Using default value: {}",
                        DEFAULT_ROLLING_FILE_MIN_SIZE);
      ...
      var overwrite =
          sparkConf
              ...
              .getOrElse(
                  () -> {
                    LOG.warn(
                        "Overwrite is not set. Using default value: {}", DEFAULT_ROLLING_OVERWRITE);
  ...
  private void writeEventToLog(SparkListenerEvent event) {
    try {
      ...
      if (lastRotationTimestamp.isAfter(Instant.MIN)
          && Instant.now().isAfter(lastRotationTimestamp.plus(rollingInterval))) {
